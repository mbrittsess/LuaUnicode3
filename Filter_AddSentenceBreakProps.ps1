<# Processes auxiliary\SentenceBreakProperty.txt
    This filter adds the Sentence_Break property.
#>
...
Begin
{
    ...
    $Reader = Get-StreamReader UCD\auxiliary\SentenceBreakProperty.txt
    $PropertyName = Normalize-PropertyName -Name Sentence_Break
    ...
    While ( ($Line = $Reader.ReadNonCommentLine()) -ne $Null )
    {
        ...
        $Cat = Normalize-PropertyValueName -PropertyName Sentence_Break -Value $Cat
        If ( $CP.Contains( '.' ) )
        {
