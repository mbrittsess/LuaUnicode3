...
Begin
{
    ...
    $Reader = Get-StreamReader UCD\auxiliary\GraphemeBreakProperty.txt
    $PropertyName = Normalize-PropertyName -Name Grapheme_Cluster_Break
    ...
    While ( ($Line = $Reader.ReadNonCommentLine()) -ne $Null )
    {
        ...
        $Cat = Normalize-PropertyValueName -PropertyName Grapheme_Cluster_Break -Value $Cat
        If ( $CP.Contains( '.' ) )
        {
